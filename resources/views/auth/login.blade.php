  ...
  <div class="login-left">
    <div class="login-form">
      ...
      <form method="POST" action="{{route('login.handle')}}">
        @csrf
        <div class="mb-3">
          <input type="email" class="form-control" name="email" value="{{old('email')}}" placeholder="Masukkan akun anda">
          @error('email')
        ...
        <div class="text-center mt-4">
          <small class="text-secondary">Copyright &copy; {{ date('Y') }} Panti Asuhan Jembatan Jalan Suci.</small>
        </div>
      </form>
    </div>
  </div>
